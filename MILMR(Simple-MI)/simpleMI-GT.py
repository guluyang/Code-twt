...
class SMI():
    def __init__(self, file_path='..\\MILframe\\data\\data\\process.mat', dr=None):
        ...
        self.f1, self.f1_std = self.KNN_CV()
...
if __name__ == '__main__':
    ...
    for path in path_li:
        smi = SMI(file_path=path, dr=None)
        # print('ins_len:', len(smi.trsed_vector_matrix[0]))  # 因为带标签，所以长度会加1
        name = path.split('/')[-1].split('.')[0]
        print('%-25s acc: $%.3f_{\\pm%.3f}$, f1: $%.3f_{\\pm%.3f}$' % (name, smi.acc / 100, smi.acc_std / 100, smi.f1 / 100, smi.f1_std / 100))
