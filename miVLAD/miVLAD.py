...
class miVLAD:
    def __init__(self,
        ...
        self.precision, self.precision_std, self.discer, self.discer_std = self.classfier_cv(classifier, k_for_cv)
...
if __name__ == '__main__':
    ...
    #
    mivlad = miVLAD(dataset_path='../MILframe/Corel/ten_corel_2/90horse_95bird_b_230+.mat', k=2, dr=None)
    print('acc: $%.3f_{\\pm%.3f}$' % (mivlad.acc, mivlad.acc_std))
    print('f1: $%.1f_{\\pm%.1f}$' % (mivlad.f1, mivlad.f1_std))
    print('recall: $%.1f_{\\pm%.1f}$' % (mivlad.recall, mivlad.recall_std))
    print('precision: $%.1f_{\\pm%.1f}$' % (mivlad.precision, mivlad.precision_std))
    print('Discernibility: $%.2f_{\\pm%.2f}$' % (mivlad.discer, mivlad.discer_std))
